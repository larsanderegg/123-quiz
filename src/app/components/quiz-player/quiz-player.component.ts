...
import { LedControlMode, Question } from '../../models';
...
export class QuizPlayerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Combine paramMap (for roundId, questionIndex, stepIndex) and detect finished route
    this.routeSub = this.route.paramMap.pipe(
      map(params => {
        const roundId = params.get('roundId');

        // Check if this is the finished route by examining the URL
        const isFinishedRoute = this.router.url.includes('/finished');

        // Only read question/step params if not on finished route
        const questionIndex = !isFinishedRoute && params.get('questionIndex')
          ? parseInt(params.get('questionIndex')!, 10)
          : 0;
        const revealStep = !isFinishedRoute && params.get('stepIndex')
          ? parseInt(params.get('stepIndex')!, 10)
          : 0;
      ...
      // If this is the finished route, skip loading questions
      if (state.isFinished) {
        this.quizFinished = true;
        this.isLoading = false;
        console.log('[Route] Showing quiz finished screen');
        return;
      }
  ...
  loadQuizData(roundId: string, initialIndex: number, initialStep: number): void {
    ...
    const roundObs$ = this.roundService.getRoundById(roundId).pipe(take(1));
    const questionsObs$ = this.questionService.getAllQuestionsByRound(roundId).pipe(take(1));

    // Unsubscribe from previous load if any
    this.dataLoadSub?.unsubscribe();

    this.dataLoadSub = forkJoin({ round: roundObs$, questions: questionsObs$ }).pipe(
      switchMap(({ round, questions }) => {
        console.log(`[loadQuizData] Round and questions received. Fetching answers for ${questions.length} questions...`);
        ...
        if (round) {
          ...
          if (round.theme) {
            console.log('[loadQuizData] Applying round theme:', round.theme);
            this.themeService.applyTheme(round.theme);
          } else {
        ...
        const questionWithAnswersObs$ = questions.map(question =>
          this.questionService.getAnswersByQuestion(question.id).pipe(
            take(1),
        ...
        if (questionWithAnswersObs$.length === 0) {
          return of([]);
        }

        return forkJoin(questionWithAnswersObs$);
      })
    ).subscribe({
      next: (questionsWithAnswers) => {
        console.log(`[loadQuizData] Data received with answers for round ${roundId}.`);

        // No buffer processing needed - Firebase returns imageUrl directly
        // Questions are already sorted by QuestionService.getAllQuestionsByRound()
        ...
        if (this.questions.length > 0) {
          ...
          if (this.currentQuestion) {
            // Safely handle answers - ensure they exist
            if (this.currentQuestion.answers && Array.isArray(this.currentQuestion.answers)) {
              const answers = this.currentQuestion.answers; // Already sorted by service
              if (answers.length > 3) {
                console.warn(`Question "${this.currentQuestion.text}" has ${answers.length} answers. Only showing first 3.`);
              }
              this.currentQuestion.answers = answers.slice(0, 3);
            } else {
          ...
          const maxAnswers = this.currentQuestion && this.currentQuestion.answers ? Math.min(this.currentQuestion.answers.length, 3) : 0;
  ...
  updateUrlState(): void {
    ...
    const url = this.router.createUrlTree(
      ['/quiz', this.activeRoundId, 'play', 'question', this.currentQuestionIndex, 'step', this.currentRevealStep]
    ).toString();
    ...
    // Reset flag immediately since location.go() doesn't trigger navigation events
    // Use setTimeout to ensure any pending synchronous operations complete first
    setTimeout(() => {
      this.navigationTriggeredByApp = false;
    }, 0);
  ...
  advanceQuizStep(): void {
    ...
    if (nextStep > maxStepOverall) {
      ...
      if (nextQuestionIndex >= this.questions.length) {
        this.quizFinished = true;
        this.updateLedState(); // Update LED for finished state
        console.log('[advanceQuizStep] Quiz finished. Navigating to finished route.');

        // Navigate to finished route
        this.router.navigate(['/quiz', this.activeRoundId, 'finished']);
